    ...
    <div class="row">
        {% for report in reports%}
			{% if report.code == 'RP-001'%}
				<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                    <a href="{{url('portfolio/showreport')}}/{{report.idReport}}" target="_blank" class="text-center"><h3>{{report.name}}</h3></a>
                    {{ partial('portfolio/partials/RP-001')}}
					<hr class="line-primary" />
                </div>
            {% elseif report.code == 'RS-003'%}
                <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                    <a href="{{url('sales/showreport')}}/{{report.idReport}}" target="_blank" class="text-center"><h3>{{report.name}}</h3></a>
